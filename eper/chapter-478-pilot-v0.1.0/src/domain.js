...
function authorize(ctx,permission){if(!ctx?.tenantId||!ctx.permissions?.includes(permission))throw new AuthorizationError("Authorization denied")}
export class RecordRepository{
 ...
 transition(ctx,id,nextState,reason){authorize(ctx,"record:transition");if(!reason||reason.trim().length<3)throw new ValidationError("transition reason required");const r=this.#records.get(id);if(!r||r.tenantId!==ctx.tenantId)throw new AuthorizationError("Record is not accessible");if(!transitions[r.state]?.has(nextState))throw new ValidationError(`Invalid transition ${r.state} -> ${nextState}`);r.state=nextState;r.version++;r.history.push({state:nextState,reason,at:new Date().toISOString(),actor:ctx.principalId??"system"});return structuredClone(r)}
 get(ctx,id){authorize(ctx,"record:read");const r=this.#records.get(id);if(!r||r.tenantId!==ctx.tenantId)throw new AuthorizationError("Record is not accessible");return structuredClone(r)}
...
export function report(ctx,repository){authorize(ctx,"report:read");return repository.list(ctx).reduce((s,r)=>{s.total++;s.byState[r.state]=(s.byState[r.state]??0)+1;return s},{total:0,byState:{}})}
