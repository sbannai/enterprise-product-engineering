...
test("XX01 authoritative records: create, update, version control and tenant isolation", async () => {
  const service = new AuthoritativeRecordService(); const requirement = req("XX01");
  ...
  await assert.rejects(service.execute(requirement,{context:context("xx01-stale"),payload:{operation:"update",tenantId:"behavioral-acceptance",id:"rec-1",expectedVersion:1,patch:{}}}),/RECORD_VERSION_CONFLICT/);
  const otherTenant=await service.execute(requirement,{context:{...context("xx01-tenant"),tenantId:"other-tenant"},payload:{operation:"get",tenantId:"other-tenant",id:"rec-1"}});
...
test("XX02 authorization: explicit allow, deny precedence and default deny", async () => {
  const service=new AuthorizationService(); const requirement=req("XX02");
  ...
  const defaultDenied=await service.execute(requirement,{context:context("xx02-default"),payload:{operation:"decide",request:{tenantId:"behavioral-acceptance",principalId:"unknown",action:"delete",resource:"record"}}});
...
test("XX03 business validation: valid input passes and rule violation is reported", async () => {
  const service=new BusinessValidationService(); const requirement=req("XX03");
...
test("XX04 audit evidence: append, integrity hash and tenant isolation", async () => {
  const service=new AuditEvidenceService(); const requirement=req("XX04");
...
test("XX05 exception handling: valid transition, retry count and invalid terminal transition", async () => {
  const service=new ExceptionHandlingService(); const requirement=req("XX05");
  ...
  await assert.rejects(service.execute(requirement,{context:context("xx05-invalid"),payload:{operation:"transition",tenantId:"behavioral-acceptance",id:"ex-1",patch:{state:"RETRYING"}}}),/EXCEPTION_INVALID_TRANSITION/);
});

test("XX06 governed reporting: publish, query filtering and tenant isolation", async () => {
  const service=new GovernedReportingService(); const requirement=req("XX06");
